...
Our leave-out-out cross-validations results indicate that the optimal bandwidth increases monotonically (See @fig-partial-local-kernel-cv-results) but this is not necessarily only due to bandwidth selection.
...
To remedy this, we fit what we call a partially local kernel logistic regression model in which we first fit a global logistic regression model with age and transplant terms and then fit a local linear kernel model on the Pearson residuals.
As we can see in @fig-partial-local-kernel-cv-results, the partial local model fits better, based on leave-one-out cross-validation, than the local kernel model.
The optimal bandwidth of `r cv_bandwidth` is shown in @fig-partial-local-kernel-glm with $\pm 5$ bandwidth to illustrate how the model fits for different bandwidth.
...
plot_bandwidth <- c(cv_bandwidth - 5, cv_bandwidth, cv_bandwidth + 5)
pred_results <- bind_rows(lapply(plot_bandwidth, function(b) {
...
#| fig-height: 3
#| fig-width: 6
#| fig-cap: "Cross validation results for partial local linear kernel logistic regression model with Gaussian kernel. The minimum deviance bandwidth for the partial local kernel model is 13.5."
...
ggplot(., aes(x = bandwidth, y = deviance)) +
  ...
  scale_x_continuous(
    breaks = c(0, 10, cv_bandwidth, 20, 30),
    labels = c(0, 10, cv_bandwidth, 20, 30),
    ) +  
...
This model first fits a global logistic regression model with age and transplant terms and then fits a local linear kernel model on the residuals.
The global model is

$$
g(\mu_i) = \beta_0 + \beta_1 \text{transplant}_i + \beta_2 \text{age}_i
$$

Then, let $r_i$ be the Pearson residuals from the global model, $r_i = \frac{y_i - \hat{\mu}_i}{\sqrt{\text{Var}(y_i)}}$.
We fit a local linear model to the Pearson residuals

$$
r_i = \theta_1(\text{age}_i) + \theta_2(\text{age}_i) \text{surgery}_i
$$

where $\theta_1(x) = \alpha_{11} + \alpha_{12} x$ and $\theta_2(x) = \alpha_{21} + \alpha_{22} x$.
This allows us to have a global term for transplant and age and a local term for bypass surgery.

## Discussion

Our analysis of the Stanford Heart Transplant dataset suggests that there is a age-varying effect for prior bypass surgery and potentially a age-varying effect for transplant.
