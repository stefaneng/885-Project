...
## Survival model
```{r}
heart_coxph <- coxph(Surv(start, stop, event) ~ surgery, data = heart)
...
log_model <- gam(event ~ s(age, by = transplant_fct, bs = "bs"), data = heart, family = binomial)
log_preds <- predict(log_model, newdata = pred_data, type = "response", se.fit = TRUE)
plot_data <- cbind(
  pred_data, prob = log_preds$fit, lower_ci = log_preds$fit - 1.96 * log_preds$se.fit, upper_ci = log_preds$fit + 1.96 * log_preds$se.fit)
ggplot(plot_data) +
  geom_line(aes(x = age, y = prob, color = transplant_fct)) +
  geom_ribbon(aes(x = age, ymin = lower_ci, ymax = upper_ci, fill = transplant_fct), alpha = 0.2) +
