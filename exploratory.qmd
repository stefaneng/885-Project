...
log_model <- gam(event ~ s(age, by = transplant, bs = "bs"), data = heart2, family = binomial)
...
ggplot(plot_data) +
  geom_line(aes(x = age, y = prob, color = transplant)) +
  geom_ribbon(aes(x = age, ymin = lower_ci, ymax = upper_ci, fill = transplant), alpha = 0.2) +
